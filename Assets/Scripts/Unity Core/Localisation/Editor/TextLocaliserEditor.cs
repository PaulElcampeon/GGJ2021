...
public class TextLocaliserSearchWindow : EditorWindow
{
    ...
    private void GetSearchResults()
    {
        ...
        foreach (KeyValuePair<string,string> element in dictionary)
        {
            ...
            {
                EditorGUILayout.BeginHorizontal("Box");
                Texture closeIcon = (Texture)Resources.Load("closeButton");

                GUIContent content = new GUIContent(closeIcon);

                if (GUILayout.Button(content, GUILayout.MaxWidth(20), GUILayout.MaxHeight(20)))
                {
                    if(EditorUtility.DisplayDialog("Remove key" + element.Key + "?", "This will remove the element from localisation, are you sure?", "Yes"))
                    {
